@extends('layouts.app')
@section('title', 'International Trip Realization Review - Manager')
@section('content')
<section class="section">
    <div class="section-header">
        <div class="section-header-back"><a href="{{ route('accounting-manager.international-trip-realization.index') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a></div>
        <h1>International Trip Realization Review - Manager</h1>
    </div>
    <div class="section-body"><div class="row"><div class="col-12"><div class="card"><div class="card-body">
        <div class="d-flex justify-content-between align-items-start mb-3">
            <div>
                <h4 class="mb-1">{{ $internationalTripRealization->number }}</h4>
                <p class="mb-0 text-muted">Cost Center: <strong>{{ optional(optional($internationalTripRealization->trip)->costCenter)->number ?? '' }} - {{ optional(optional($internationalTripRealization->trip)->costCenter)->name ?? '' }}</strong></p>
            </div>
            <div>
        ...
        <h5>International Trip</h5>
            <div class="table-responsive mb-3">
                <table class="table table-sm table-bordered">
                    <tbody>
                        <tr><th style="width:200px">International Trip Number</th><td>{{ optional($internationalTripRealization->trip)->number }}</td></tr>
                        <tr><th>Document Number</th><td>{{ optional($internationalTripRealization->trip)->document_number }}</td></tr>
                        <tr><th>Cost Center</th><td>{{ optional($internationalTripRealization->costCenter)->number }} - {{ optional($internationalTripRealization->costCenter)->name }}</td></tr>
                    </tbody>
                </table>
            </div>
        <h5>Submitted By</h5>
        <div class="table-responsive mb-3"><table class="table table-sm table-bordered"><tbody>
            <tr><th style="width:200px">Employee ID</th><td>{{ optional($internationalTripRealization->trip->user ?? $internationalTripRealization->trip)->employee_id ?? '-' }}</td></tr>
            <tr><th>Name</th><td>{{ optional($internationalTripRealization->trip->user ?? $internationalTripRealization->trip)->name ?? '-' }}</td></tr>
            <tr><th>Email</th><td>{{ optional($internationalTripRealization->trip->user ?? $internationalTripRealization->trip)->email ?? '-' }}</td></tr>
            <tr><th>Department</th><td>{{ optional(optional($internationalTripRealization->trip->user ?? $internationalTripRealization->trip)->department)->department ?? '-' }}</td></tr>
            <tr><th>Position</th><td>{{ optional(optional($internationalTripRealization->trip->user ?? $internationalTripRealization->trip)->position)->position ?? '-' }}</td></tr>
        </tbody></table></div>

        <h5>Uploaded Documents</h5>
        <div class="row mb-4"><div class="col-12"><ul class="list-group">
            @php $files=['transfer_evidence'=>'Transfer Evidence','other_document'=>'Other Document']; @endphp
            @foreach($files as $field=>$fileLabel)
            @if(!empty($internationalTripRealization->{$field}))
            <li class="list-group-item d-flex justify-content-between align-items-center"><div><strong>{{ $fileLabel }}</strong><div class="text-muted small">{{ basename($internationalTripRealization->{$field}) }}</div></div><div><a href="{{ asset('storage/'.$internationalTripRealization->{$field}) }}" target="_blank" class="btn btn-sm btn-outline-primary">View</a></div></li>
            @else
            @if(in_array($field, ['transfer_evidence']))
        ...
        <div class="row">
            <div class="col-md-6"><div class="card"><div class="card-header">@php $totalRevisions=$internationalTripRealization->revisions()->count();$maxRevisions=3; @endphp
                                        <h4><i class="fas fa-exclamation-circle mr-2"></i>Revisions ({{ $totalRevisions }}/{{ $maxRevisions }})</h4></div><div class="card-body">
                @if($internationalTripRealization->revisions()->count()===0)<p class="text-muted">No revisions requested yet.</p>
                @else<ul class="list-unstyled">
                    @foreach($internationalTripRealization->revisions()->with(['user','status'])->orderByDesc('revision_at')->get() as $rev)
            ...
            <div class="col-md-6"><div class="card"><div class="card-header"><h4><i class="fas fa-check-circle mr-2"></i>Approval Chain</h4></div><div class="card-body"><div class="timeline">
                ...
                @empty<p class="text-muted">No approval chain available.</p>@endforelse
                        ...
                        @php $hasRejected=$internationalTripRealization->approvals()->where('approval_status_id',2)->exists(); @endphp
                        ...
                        <div class="card-body">
                            ...
                            <a href="{{ route('accounting-manager.international-trip-realization.index') }}" class="btn btn-light">Back</a>
                        </div></div></div>
    </div></div></div></div></div>
</section>
...
<div class="modal fade" id="approveModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    ...
    <form action="{{ route('accounting-manager.international-trip-realization.approve', $internationalTripRealization) }}" method="POST">@csrf
        <div class="modal-body"><p>Are you sure you want to approve this International Trip Realization?</p><div class="form-group"><label>Remarks (Optional)</label><textarea class="form-control" name="remark" rows="3"></textarea></div></div>
...
<div class="modal fade" id="rejectModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    ...
    <form action="{{ route('accounting-manager.international-trip-realization.reject', $internationalTripRealization) }}" method="POST">@csrf
