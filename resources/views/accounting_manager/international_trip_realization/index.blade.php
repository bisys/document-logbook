...
@section('title', 'International Trip Realization - Manager Review')

@section('content')
<section class="section">
    <div class="section-header">
        <h1>International Trip Realization - Manager Review</h1>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-header-action">
                            <form id="bulk-approve-form" action="{{ route('accounting-manager.international-trip-realization.bulk-approve') }}" method="POST">
                                @csrf
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="text-center">
                                            @if(optional($internationalTripRealization->status)->slug === 'waiting-approval-manager')
                                            <div class="custom-checkbox custom-control">
                                        ...
                                        <td>
                                            @php
                                            $staffApproval = $internationalTripRealization->approvals()->where('approval_role_id', 1)->first();
                                            @endphp
                                            @if($staffApproval)
                                            @php
                                        ...
                                        <td>
                                            <a href="{{ route('accounting-manager.international-trip-realization.show', $internationalTripRealization) }}" class="btn btn-sm btn-primary">Review</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
...
<script>iziToast.success({message: '{{ session()->get("success") }}',position: 'topRight'});</script>
@endif
@if(session()->has('error'))
<script>iziToast.warning({message: '{{ session()->get("error") }}',position: 'topRight'});</script>
@endif
...
<script>
    $(document).ready(function() {
        function updateBulkApproveButton() {
            ...
            if (selectedCount > 0) { $('#btn-bulk-approve').show(); } else { $('#btn-bulk-approve').hide(); }
        }
        $('#table-1').on('change', 'input[type="checkbox"]', function() { setTimeout(updateBulkApproveButton, 50); });
        $('#btn-bulk-approve').click(function() {
            var selectedIds = [];
            $('.doc-checkbox:checked').each(function() { selectedIds.push($(this).val()); });
            if (selectedIds.length === 0) return;
            swal({title: 'Approve Selected Documents?',text: 'You are about to approve ' + selectedIds.length + ' document(s). You can add an optional remark below:',content: {element: "input",attributes: {placeholder: "Optional Remark",type: "text",},},icon: 'warning',buttons: true,})
            .then((remark) => {
                if (remark !== null) {
                    $('#bulk-remark').val(remark);
                    $('#bulk-inputs').empty();
                    selectedIds.forEach(function(id) { $('#bulk-inputs').append('<input type="hidden" name="document_ids[]" value="' + id + '">'); });
