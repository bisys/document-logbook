@extends('layouts.app')
@section('title', 'Cash Advance Realization Review - Manager')
@section('content')
<section class="section">
    <div class="section-header">
        <div class="section-header-back"><a href="{{ route('accounting-manager.cash-advance-realization.index') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a></div>
        <h1>Cash Advance Realization Review - Manager</h1>
    </div>
    <div class="section-body"><div class="row"><div class="col-12"><div class="card"><div class="card-body">
        <div class="d-flex justify-content-between align-items-start mb-3">
            ...
            <div>
                @php $statusText=optional($cashAdvanceRealization->status)->status??'Unknown'; @endphp
        ...
        <div class="table-responsive mb-3"><table class="table table-sm table-bordered"><tbody>
            ...
            <tr><th>Name</th><td>{{ optional($cashAdvanceRealization->draw->user ?? $cashAdvanceRealization->draw)->name ?? '-' }}</td></tr>
            <tr><th>Email</th><td>{{ optional($cashAdvanceRealization->draw->user ?? $cashAdvanceRealization->draw)->email ?? '-' }}</td></tr>
            <tr><th>Department</th><td>{{ optional(optional($cashAdvanceRealization->draw->user ?? $cashAdvanceRealization->draw)->department)->department ?? '-' }}</td></tr>
            <tr><th>Position</th><td>{{ optional(optional($cashAdvanceRealization->draw->user ?? $cashAdvanceRealization->draw)->position)->position ?? '-' }}</td></tr>
        </tbody></table></div>
        ...
        <div class="row mb-4"><div class="col-12"><ul class="list-group">
            ...
            @else
            @if(in_array($field, ['car_form','original_invoice','copy_invoice','budget_plan']))
                                ...
                                <div class="alert alert-success mb-0">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-white">Date: <strong>{{ optional($cashAdvanceRealization->hardfile_submitted_at)->format('d M Y H:i') }}</strong></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="alert alert-success mb-0">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-white">Received by: <strong>{{ optional($cashAdvanceRealization->hardfileReceivedByUser)->name ?? '-' }}</strong></span><br>
                                            <span class="text-white">Date: <strong>{{ optional($cashAdvanceRealization->hardfile_received_at)->format('d M Y H:i') }}</strong></span>
                                        </div>
                                    </div>
                                </div>
                            @elseif($cashAdvanceRealization->is_hardfile_submitted)
                                <div class="alert alert-warning mb-0">
        ...
        <div class="row">
            <div class="col-md-6"><div class="card"><div class="card-header">@php $totalRevisions=$cashAdvanceRealization->revisions()->count();$maxRevisions=3; @endphp
            ...
            <div class="col-md-6"><div class="card"><div class="card-header"><h4><i class="fas fa-check-circle mr-2"></i>Approval Chain</h4></div><div class="card-body"><div class="timeline">
                ...
                @empty<p class="text-muted">No approval chain available.</p>@endforelse
                        ...
                        @php $hasRejected=$cashAdvanceRealization->approvals()->where('approval_status_id',2)->exists(); @endphp
                        ...
                        <div class="card-body">
                            ...
                            <a href="{{ route('accounting-manager.cash-advance-realization.index') }}" class="btn btn-light">Back</a>
                        </div></div></div>
    </div></div></div></div></div>
</section>
...
<div class="modal fade" id="approveModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    ...
    <form action="{{ route('accounting-manager.cash-advance-realization.approve', $cashAdvanceRealization) }}" method="POST">@csrf
        <div class="modal-body"><p>Are you sure you want to approve this Cash Advance Realization?</p><div class="form-group"><label>Remarks (Optional)</label><textarea class="form-control" name="remark" rows="3"></textarea></div></div>
...
<div class="modal fade" id="rejectModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    ...
    <form action="{{ route('accounting-manager.cash-advance-realization.reject', $cashAdvanceRealization) }}" method="POST">@csrf
