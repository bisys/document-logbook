...
@section('title', 'International Trip Realization Detail')
...
<section class="section">
    <div class="section-header">
        ...
        <h1>International Trip Realization Detail</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
            <div class="breadcrumb-item"><a href="{{ route('user.international-trip-realization.index') }}">International Trip Realization</a></div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-sm table-bordered">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ optional($internationalTripRealization->user)->employee_id }}</td>
                                    </tr>
                                    <tr>
                                        <th>Name</th>
                                        <td>{{ optional($internationalTripRealization->user)->name }}</td>
                                    </tr>
                                    <tr>
                                        <th>Email</th>
                                        <td>{{ optional($internationalTripRealization->user)->email }}</td>
                                    </tr>
                                    <tr>
                                        <th>Department</th>
                                        <td>{{ optional(optional($internationalTripRealization->user)->department)->department }}</td>
                                    </tr>
                                    <tr>
                                        <th>Position</th>
                                        <td>{{ optional(optional($internationalTripRealization->user)->position)->position }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                            ...
                            <div class="col-12">
                                <ul class="list-group">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div>
                                            <strong>{{ $label }}</strong>
                                            <div class="text-muted small">{{ basename($internationalTripRealization->{$field}) }}</div>
                                        </div>
                                        <div>
                                            <a href="{{ asset('storage/' . $internationalTripRealization->{$field}) }}" target="_blank" class="btn btn-sm btn-outline-primary">View</a>
                                        </div>
                                    </li>
                                    @else
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <ul class="list-unstyled activity-timeline">
                                            @forelse($internationalTripRealization->revisions()->with(['user','status'])->orderByDesc('revision_at')->get() as $rev)
                                            <li class="media mb-3">
                                        ...
                                        <div class="timeline">
                                            ...
                                            <div class="mb-4">
                                                <div class="d-flex justify-content-between align-items-start">
                                                    <div><strong>{{ $item['role']->name }}</strong></div>
                                                    <div>
                        ...
                        <div class="mt-3">
                            <a href="{{ route('user.international-trip-realization.index') }}" class="btn btn-light">Back</a>
                            @if($canEdit)
                            <a href="{{ route('user.international-trip-realization.edit', $internationalTripRealization->id) }}" class="btn btn-primary">
...
<!-- Revision Modals -->
@foreach($internationalTripRealization->revisions()->where('revision_status_id', 1)->get() as $revision)
<div class="modal fade" id="revisionModal{{ $revision->id }}" tabindex="-1" role="dialog" aria-labelledby="revisionModalLabel{{ $revision->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.international-trip-realization.submit-revision', [$internationalTripRealization, $revision]) }}" method="POST" enctype="multipart/form-data" id="revision-form">
                @csrf
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <small class="d-block text-muted mb-2">
                                    @if(!empty($internationalTripRealization->{$field}))
                                    Current: {{ basename($internationalTripRealization->{$field}) }}
                                    @else
                                    Not yet uploaded
                                    @endif
                                </small>
