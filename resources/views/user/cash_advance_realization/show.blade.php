...
@section('title', 'Cash Advance Realization Detail')
...
<section class="section">
    <div class="section-header">
        ...
        <h1>Cash Advance Realization Detail</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
            <div class="breadcrumb-item"><a href="{{ route('user.cash-advance-realization.index') }}">Cash Advance Realization</a></div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-sm table-bordered">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ optional($cashAdvanceRealization->user)->employee_id }}</td>
                                    </tr>
                                    <tr>
                                        <th>Name</th>
                                        <td>{{ optional($cashAdvanceRealization->user)->name }}</td>
                                    </tr>
                                    <tr>
                                        <th>Email</th>
                                        <td>{{ optional($cashAdvanceRealization->user)->email }}</td>
                                    </tr>
                                    <tr>
                                        <th>Department</th>
                                        <td>{{ optional(optional($cashAdvanceRealization->user)->department)->department }}</td>
                                    </tr>
                                    <tr>
                                        <th>Position</th>
                                        <td>{{ optional(optional($cashAdvanceRealization->user)->position)->position }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                            ...
                            <div class="col-12">
                                <ul class="list-group">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div>
                                            <strong>{{ $label }}</strong>
                                            <div class="text-muted small">{{ basename($cashAdvanceRealization->{$field}) }}</div>
                                        </div>
                                        <div>
                                            <a href="{{ asset('storage/' . $cashAdvanceRealization->{$field}) }}" target="_blank" class="btn btn-sm btn-outline-primary">View</a>
                                        </div>
                                    </li>
                                    @else
                                    @if(in_array($field, ['car_form','original_invoice','copy_invoice']))
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div>
                                            <strong>{{ $label }}</strong>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <ul class="list-unstyled activity-timeline">
                                            @forelse($cashAdvanceRealization->revisions()->with(['user','status'])->orderByDesc('revision_at')->get() as $rev)
                                            <li class="media mb-3">
                        ...
                        <div class="mt-3">
                            <a href="{{ route('user.cash-advance-realization.index') }}" class="btn btn-light">Back</a>
                            @if($canEdit)
                            <a href="{{ route('user.cash-advance-realization.edit', $cashAdvanceRealization->id) }}" class="btn btn-primary">
...
<!-- Revision Modals -->
@foreach($cashAdvanceRealization->revisions()->where('revision_status_id', 1)->get() as $revision)
<div class="modal fade" id="revisionModal{{ $revision->id }}" tabindex="-1" role="dialog" aria-labelledby="revisionModalLabel{{ $revision->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.cash-advance-realization.submit-revision', [$cashAdvanceRealization, $revision]) }}" method="POST" enctype="multipart/form-data" id="revision-form">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="document_number" name="document_number" value="{{ $cashAdvanceRealization->document_number }}">
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <small class="d-block text-muted mb-2">
                                    @if(!empty($cashAdvanceRealization->{$field}))
                                    Current: {{ basename($cashAdvanceRealization->{$field}) }}
                                    @else
                                    Not yet uploaded
                                    @endif
                                </small>
...
@if(!$cashAdvanceRealization->is_hardfile_submitted && $staffApproved)
<div class="modal fade" id="submitHardfileModal" tabindex="-1" role="dialog" aria-labelledby="submitHardfileModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.cash-advance-realization.submit-hardfile', $cashAdvanceRealization) }}" method="POST" id="submit-hardfile-form">
                @csrf
                <div class="modal-body">
