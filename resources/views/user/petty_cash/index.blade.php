...
<section class="section">
    <div class="section-header">
        ...
        <div class="section-header-button">
            <a href="{{ route('user.petty-cash.create') }}" class="btn btn-primary">Create New</a>
        </div>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
            <div class="breadcrumb-item"><a href="#">Petty Cash</a></div>
            <div class="breadcrumb-item">
                @if($statusFilter === 'all') All Petty Cash @elseif($statusFilter === 'waiting-approval-staff') Waiting Approval Staff @elseif($statusFilter === 'waiting-approval-manager') Waiting Approval Manager @elseif($statusFilter === 'waiting-approval-gm') Waiting Approval GM @elseif($statusFilter === 'waiting-revision') Waiting Revision @elseif($statusFilter === 'fully-approved') Fully Approved @endif
            </div>
        </div>
    </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4>
                            @if($statusFilter === 'all') All Petty Cash
                        ...
                        <div class="card-header-action">
                            <form id="bulk-submit-form" action="{{ route('user.petty-cash.bulk-submit-hardfile') }}" method="POST">
                                @csrf
                                    ...
                                    <tr>
                                        <td class="text-center">
                                            @php
                                                ...
                                                $hasStaffApproved = $staffRole ? $pettyCash->approvals->where('approval_role_id', $staffRole->id)->where('approval_status_id', 1)->isNotEmpty() : false;
                                            @endphp
                                            @if(!$pettyCash->is_hardfile_submitted && $hasStaffApproved)
                                            <div class="custom-checkbox custom-control">
                                                <input type="checkbox" data-checkboxes="mygroup" class="custom-control-input doc-checkbox" id="checkbox-{{ $pettyCash->id }}" value="{{ $pettyCash->id }}">
                                                <label for="checkbox-{{ $pettyCash->id }}" class="custom-control-label">&nbsp;</label>
                                            </div>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td>{{ optional($pettyCash->costCenter)->number }} - {{ optional($pettyCash->costCenter)->name }}</td>
                                        <td>
                                            <span class="badge badge-info">{{ $pettyCash->revisions()->count() }} revisions</span>
                                        </td>
                                        <td>
                                            @php
