...
@section('title', 'International Trip Detail')
...
<section class="section">
    <div class="section-header">
        ...
        <h1>International Trip Detail</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
            <div class="breadcrumb-item"><a href="{{ route('user.international-trip.index') }}">International Trip</a></div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                            ...
                            <table class="table table-sm table-bordered">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{{ optional($internationalTrip->user)->employee_id }}</td>
                                    </tr>
                                    <tr>
                                        <th>Name</th>
                                        <td>{{ optional($internationalTrip->user)->name }}</td>
                                    </tr>
                                    <tr>
                                        <th>Email</th>
                                        <td>{{ optional($internationalTrip->user)->email }}</td>
                                    </tr>
                                    <tr>
                                        <th>Department</th>
                                        <td>{{ optional(optional($internationalTrip->user)->department)->department }}</td>
                                    </tr>
                                    <tr>
                                        <th>Position</th>
                                        <td>{{ optional(optional($internationalTrip->user)->position)->position }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-12">
                                <ul class="list-group">
                                    ...
                                    'summary_bussiness_trip' => 'Summary Bussiness Trip',
                                    'overseas_allowance_form' => 'Overseas Allowance Form',
                                    'bussiness_trip_allowance' => 'Bussiness Trip Allowance',
                                    'rate' => 'Rate',
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div>
                                            <strong>{{ $label }}</strong>
                                            <div class="text-muted small">{{ basename($internationalTrip->{$field}) }}</div>
                                        </div>
                                        <div>
                                            <a href="{{ asset('storage/' . $internationalTrip->{$field}) }}" target="_blank" class="btn btn-sm btn-outline-primary">View</a>
                                        </div>
                                    </li>
                                    @else
                                                ...
                                                <div class="alert alert-success mb-0">
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <div>
                                                            ...
                                                            <span class="text-white">Date: <strong>{{ optional($internationalTrip->hardfile_submitted_at)->format('d M Y H:i') }}</strong></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="alert alert-success mb-0">
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <div>
                                                            ...
                                                            <span class="text-white">Received by: <strong>{{ optional($internationalTrip->hardfileReceivedByUser)->name ?? '-' }}</strong></span><br>
                                                            <span class="text-white">Date: <strong>{{ optional($internationalTrip->hardfile_received_at)->format('d M Y H:i') }}</strong></span>
                                                        </div>
                                                    </div>
                                                </div>
                                            @elseif($internationalTrip->is_hardfile_submitted)
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <ul class="list-unstyled activity-timeline">
                                            @forelse($internationalTrip->revisions()->with(['user','status'])->orderByDesc('revision_at')->get() as $rev)
                                            <li class="media mb-3">
                        ...
                        <div class="mt-3">
                            <a href="{{ route('user.international-trip.index') }}" class="btn btn-light">Back</a>
                            @if($canEdit)
                            <a href="{{ route('user.international-trip.edit', $internationalTrip->id) }}" class="btn btn-primary">
...
<!-- Revision Modals -->
@foreach($internationalTrip->revisions()->where('revision_status_id', 1)->get() as $revision)
<div class="modal fade" id="revisionModal{{ $revision->id }}" tabindex="-1" role="dialog" aria-labelledby="revisionModalLabel{{ $revision->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.international-trip.submit-revision', [$internationalTrip, $revision]) }}" method="POST" enctype="multipart/form-data" id="revision-form">
                @csrf
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="document_number" name="document_number" value="{{ $internationalTrip->document_number }}">
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <small class="d-block text-muted mb-2">
                                    @if(!empty($internationalTrip->{$field}))
                                    Current: {{ basename($internationalTrip->{$field}) }}
                                    @else
                                    Not yet uploaded
                                    @endif
                                </small>
...
@if(!$internationalTrip->is_hardfile_submitted && $staffApproved)
<div class="modal fade" id="submitHardfileModal" tabindex="-1" role="dialog" aria-labelledby="submitHardfileModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('user.international-trip.submit-hardfile', $internationalTrip) }}" method="POST" id="submit-hardfile-form">
                @csrf
                <div class="modal-body">
