    ...
    <div class="section-body">
        <div class="row"><div class="col-12"><div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4>International Trip List</h4>
                <a href="{{ route('user.international-trip.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i> Create New</a>
            </div>
            <div class="card-body">
                ...
                <div class="table-responsive"><table class="table table-striped">
                    ...
                    <tbody>
                        @forelse($internationalTrips as $i => $internationalTrip)
                        <tr>
                            <td>{{ $i+1 }}</td>
                            <td>{{ $internationalTrip->document_number }}</td>
                            <td>{{ optional($internationalTrip->user)->name }}</td>
                            <td>{{ optional($internationalTrip->costCenter)->number }}</td>
                            <td>{{ $internationalTrip->revisions->count() }}</td>
                            <td>
                                @php $st=optional($internationalTrip->status)->status??'Unknown'; @endphp
                            ...
                            <td>{{ $internationalTrip->created_at->format('d M Y H:i') }}</td>
                            <td><a href="{{ route('user.international-trip.show', $internationalTrip) }}" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a></td>
                        </tr>
                        @empty
