            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4>All Petty Cash</h4>
                        <div class="card-header-action">
                            <span class="badge badge-primary">{{ $pettyCashes->count() }} Total</span>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <th>Hardfile</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge badge-info">{{ $pettyCash->revisions()->count() }}</span>
                                        </td>
                                        <td>
                                            @php
                                            $approvalCount = $pettyCash->approvals()->count();
                                            $approvedCount = $pettyCash->approvals()->where('approval_status_id', 3)->count();
                                            @endphp
                                            <small>{{ $approvedCount }}/3</small>
                                        </td>
                                        <td>
                                            {{ optional($pettyCash->created_at)->format('d M Y H:i') }}
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.petty-cash.show', $pettyCash) }}" class="btn btn-sm btn-primary">View</a>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="8" class="text-center text-muted">No petty cash found</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
