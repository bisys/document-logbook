...
@section('title', 'International Trip Realization Details - Admin')

@section('content')
<section class="section">
    <div class="section-header">
        <div class="section-header-back">
            <a href="{{ route('admin.international-trip-realization.index') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
        </div>
        <h1>International Trip Realization Details - Admin</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
            <div class="breadcrumb-item"><a href="{{ route('admin.international-trip-realization.index') }}">International Trip Realization</a></div>
    ...
    <div class="section-body">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                ...
                                <p class="mb-0 text-muted">Cost Center: <strong>{{ optional($internationalTripRealization->costCenter)->number }} - {{ optional($internationalTripRealization->costCenter)->name }}</strong></p>
                            </div>
                            <div>
                        ...
                        <div class="table-responsive mb-3">
                            <table class="table table-sm table-bordered">
                                <tbody>
                                    <tr><th style="width:200px">Employee ID</th><td>{{ optional($internationalTripRealization->user)->employee_id }}</td></tr>
                                    <tr><th>Name</th><td>{{ optional($internationalTripRealization->user)->name }}</td></tr>
                                    <tr><th>Email</th><td>{{ optional($internationalTripRealization->user)->email }}</td></tr>
                                    <tr><th>Department</th><td>{{ optional(optional($internationalTripRealization->user)->department)->department }}</td></tr>
                                    <tr><th>Position</th><td>{{ optional(optional($internationalTripRealization->user)->position)->position }}</td></tr>
                                </tbody>
                            </table>
                        </div>
                            ...
                            <div class="col-12">
                                <ul class="list-group">
                                    ...
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div><strong>{{ $label }}</strong><div class="text-muted small">{{ basename($internationalTripRealization->{$field}) }}</div></div>
                                        <div><a href="{{ asset('storage/' . $internationalTripRealization->{$field}) }}" target="_blank" class="btn btn-sm btn-outline-primary">View</a></div>
                                    </li>
                                    @else
                                    @if(in_array($field, ['transfer_evidence']))
                                    <li class="list-group-item d-flex justify-content-between align-items-center">
                                        <div><strong>{{ $label }}</strong><div class="text-muted small">Not uploaded</div></div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-header"><h4>Revisions Summary</h4></div>
                                    <div class="card-body">
                                        @if($internationalTripRealization->revisions()->count() === 0)
                            ...
                            <div class="col-md-6">
                                <div class="card">
                                    <div class="card-header"><h4>Approval Chain</h4></div>
                                    <div class="card-body">
                                        @php
                                        $steps = ['Accounting Staff','Accounting Manager'];
                                        $approvals = $internationalTripRealization->approvals()->with(['user','status','role'])->get();
                                        @endphp
                                        <div class="timeline">
                        ...
                        <div class="mt-4">
                            <div class="card">
                                <div class="card-header"><h4>Admin Actions</h4></div>
                                <div class="card-body">
                                    <button class="btn btn-info" data-toggle="modal" data-target="#updateStatusModal">
                                        <i class="fas fa-cog"></i> Force Update Status
                                    </button>
                                    <a href="{{ route('admin.international-trip-realization.index') }}" class="btn btn-light">Back</a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card-body">
            <div class="alert alert-success mb-0"><div class="d-flex align-items-center"><i class="fas fa-check-circle fa-2x mr-3"></i><div><strong>Hardfile Received</strong><br><span class="text-muted">Received by: <strong>{{ optional($internationalTripRealization->hardfileReceivedByUser)->name ?? '-' }}</strong></span><br><span class="text-muted">Date: <strong>{{ $internationalTripRealization->hardfile_received_at->format('d M Y H:i') }}</strong></span></div></div></div>
        </div></div></div>
...
<div class="modal fade" id="updateStatusModal" tabindex="-1" role="dialog" aria-labelledby="updateStatusModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.international-trip-realization.update-status', $internationalTripRealization) }}" method="POST">
                @csrf
...
@if($errors->any())@foreach($errors->all() as $error)<script>iziToast.error({message:'{{ $error }}',position:'topRight'});</script>@endforeach @endif
