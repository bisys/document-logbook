@extends('layouts.app')
@section('title', 'Cash Advance Realization Review - Admin')
@section('content')
<section class="section">
    <div class="section-header">
        <div class="section-header-back"><a href="{{ route('admin.cash-advance-realization.index') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a></div>
        <h1>Cash Advance Realization Review - Admin</h1>
    </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6"><div class="card"><div class="card-header"><h4>Approval Chain</h4></div><div class="card-body"><div class="timeline">
                ...
                @empty<p class="text-muted">No approval chain available.</p>@endforelse
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.cash-advance-realization.update-status', $cashAdvanceRealization) }}" method="POST" class="form-inline">
                                @csrf
                                <select name="document_status_id" class="form-control mr-2">
                                    @foreach(\App\Models\DocumentStatus::all() as $s)
                                    <option value="{{ $s->id }}" {{ $cashAdvanceRealization->document_status_id==$s->id?'selected':'' }}>{{ $s->status }}</option>
                                    @endforeach
                                </select>
                                <input type="text" name="remark" class="form-control mr-2" placeholder="Remark (optional)">
                                <button type="submit" class="btn btn-primary">Update Status</button>
                            </form>
                            <a href="{{ route('admin.cash-advance-realization.index') }}" class="btn btn-light mt-2">Back</a>
                        </div></div></div>
    </div></div></div></div></div>
</section>
