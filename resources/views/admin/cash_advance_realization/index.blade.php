...
@section('title', 'Cash Advance Realization - Admin')

@section('content')
<section class="section">
    <div class="section-header">
        <h1>Cash Advance Realization - Admin</h1>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4>All Cash Advance Realization</h4>
                        <div class="card-header-action">
                            <span class="badge badge-primary">{{ $cashAdvanceRealizations->count() }} Total</span>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped" id="table-1">
                                ...
                                <tbody>
                                    @forelse($cashAdvanceRealizations as $cashAdvanceRealization)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            <span class="badge badge-info">{{ $cashAdvanceRealization->revisions()->count() }}</span>
                                        </td>
                                        <td>
                                            @php
                                            $approvalCount = $cashAdvanceRealization->approvals()->count();
                                            $approvedCount = $cashAdvanceRealization->approvals()->where('approval_status_id', 3)->count();
                                            @endphp
                                            <small>{{ $approvedCount }}/3</small>
                                        </td>
                                        <td>
                                            {{ optional($cashAdvanceRealization->created_at)->format('d M Y H:i') }}
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.cash-advance-realization.show', $cashAdvanceRealization) }}" class="btn btn-sm btn-primary">View</a>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr>
                                        <td colspan="8" class="text-center text-muted">No cash advance realization found</td>
                                    </tr>
                                    @endforelse
                                </tbody>
                            </table>
