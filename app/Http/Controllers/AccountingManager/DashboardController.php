...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $pendingSP = (clone $spQuery)->whereHas('status', function($q){ $q->where('slug', 'waiting-approval-manager'); })->count();
        $pendingPC = (clone $pcQuery)->whereHas('status', function($q){ $q->where('slug', 'waiting-approval-manager'); })->count();
        $pendingIT = (clone $itQuery)->whereHas('status', function($q){ $q->where('slug', 'waiting-approval-manager'); })->count();
        $pendingCAD = (clone $cadQuery)->whereHas('status', function($q){ $q->where('slug', 'waiting-approval-manager'); })->count();
        $pendingCAR = (clone $carQuery)->whereHas('status', function($q){ $q->where('slug', 'waiting-approval-manager'); })->count();
        ...
        return view('accounting_manager.dashboard', compact(
