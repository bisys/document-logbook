...
class PettyCashController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('accounting_manager.petty_cash.index', compact('pettyCashes', 'statusFilter', 'counts'));
    }
    ...
    public function show(PettyCash $pettyCash)
    {
        ...
        return view('accounting_manager.petty_cash.show', compact('pettyCash', 'approvalChain'));
    }
    ...
    public function approve(Request $request, PettyCash $pettyCash)
    {
        ...
        try {
            DB::transaction(function () use ($pettyCash, $validated) {
                ...
                if ($nextStatus) $pettyCash->update(['document_status_id' => $nextStatus->id]);
                $pettyCash->approvals()->save($approval);
            });
            $this->notificationService->notifyDocumentApproved($pettyCash, Auth::user(), 'Accounting Manager', $validated['remark'] ?? null, 2);
            return redirect()->route('accounting-manager.petty-cash.show', $pettyCash)->with('success', 'Petty Cash approved successfully.');
        } catch (\Exception $e) {
            return redirect()->route('accounting-manager.petty-cash.show', $pettyCash)->with('error', $e->getMessage());
        }
    }
    ...
    public function reject(Request $request, PettyCash $pettyCash)
    {
        ...
        try {
            ...
            $this->notificationService->notifyDocumentRejected($pettyCash, Auth::user(), 'Accounting Manager', $validated['remark'], 2);
            return redirect()->route('accounting-manager.petty-cash.show', $pettyCash)->with('success', 'Petty Cash rejected successfully.');
        } catch (\Exception $e) {
            return redirect()->route('accounting-manager.petty-cash.show', $pettyCash)->with('error', $e->getMessage());
        }
    }
