...
class InternationalTripRealizationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('accounting_manager.international_trip_realization.index', compact('internationalTripRealizations', 'statusFilter', 'counts'));
    }
    ...
    public function show(InternationalTripRealization $internationalTripRealization)
    {
        ...
        return view('accounting_manager.international_trip_realization.show', compact('internationalTripRealization', 'approvalChain'));
    }
    ...
    public function approve(Request $request, InternationalTripRealization $internationalTripRealization)
    {
        ...
        try {
            DB::transaction(function () use ($internationalTripRealization, $validated) {
                ...
                // Final approval at Manager (sequence 2)
                $fullyApprovedStatus = DocumentStatus::where('slug', 'fully-approved')->first();
                if ($fullyApprovedStatus) {
                    $internationalTripRealization->update(['document_status_id' => $fullyApprovedStatus->id]);
                }
                $internationalTripRealization->approvals()->save($approval);
            });
            $this->notificationService->notifyDocumentApproved($internationalTripRealization, Auth::user(), 'Accounting Manager', $validated['remark'] ?? null, 2);
            return redirect()->route('accounting-manager.international-trip-realization.show', $internationalTripRealization)->with('success', 'International Trip Realization approved successfully.');
        } catch (\Exception $e) {
            return redirect()->route('accounting-manager.international-trip-realization.show', $internationalTripRealization)->with('error', $e->getMessage());
        }
    }
    ...
    public function reject(Request $request, InternationalTripRealization $internationalTripRealization)
    {
        ...
        try {
            ...
            $this->notificationService->notifyDocumentRejected($internationalTripRealization, Auth::user(), 'Accounting Manager', $validated['remark'], 2);
            return redirect()->route('accounting-manager.international-trip-realization.show', $internationalTripRealization)->with('success', 'International Trip Realization rejected successfully.');
        } catch (\Exception $e) {
            return redirect()->route('accounting-manager.international-trip-realization.show', $internationalTripRealization)->with('error', $e->getMessage());
        }
    }
        ...
        foreach ($validated['document_ids'] as $docId) {
            try {
                ...
                DB::transaction(function () use ($doc, $validated) {
                    ...
                    if ($doc->approvals()->where('approval_role_id', $userRole->id)->where('approval_status_id', 1)->exists()) throw new \Exception('Already approved by your role.');
                    $approval = new Approval(['user_id' => Auth::user()->id, 'approval_role_id' => $userRole->id, 'approval_status_id' => 1, 'remark' => $validated['remark'] ?? null, 'approval_at' => now()]);
                    $fullyApprovedStatus = DocumentStatus::where('slug', 'fully-approved')->first();
                    if ($fullyApprovedStatus) $doc->update(['document_status_id' => $fullyApprovedStatus->id]);
                    $doc->approvals()->save($approval);
                });
                $this->notificationService->notifyDocumentApproved($doc, Auth::user(), 'Accounting Manager', $validated['remark'] ?? null, 2);
                $successCount++;
            } catch (\Exception $e) {
